...
int main() {
    ...
    auto find_valid_point = [&](int& x, int& y) {
        do {
            x = pos_dis(gen);
            y = pos_dis(gen);
    ...
    // 查找起点
    find_valid_point(points.startX, points.startY);

    // 查找终点，并确保与起点距离足够远
    double min_distance = header.width / 2.0; // 最小距离阈值
    double distance = 0;
    do {
        find_valid_point(points.endX, points.endY);
        double dx = points.startX - points.endX;
        double dy = points.startY - points.endY;
